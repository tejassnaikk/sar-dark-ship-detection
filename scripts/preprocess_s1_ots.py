"""
OTS scene GRD preprocessing pipeline.

Scene: S1A_IW_GRDH_1SDV_20260517T172536_20260517T172601_064560_0821BD_460B.SAFE
Date:  2026-05-17  |  Footprint: 52.893–54.799°N, 2.937–7.335°E
...
AOIs (density-search verified, wind-farm check passed):
  ots_aoi_a: 53.30–53.80°N, 3.80–4.50°E  (Terschelling TSS western corridor)
  ots_aoi_b: 53.50–54.05°N, 5.00–5.60°E  (central-east; eastern edge pulled to
              5.60°E, giving 24 km margin from Gemini offshore wind farm at 5.90°E)

Note: interior ocean AOIs produce very few >80%-black chips — that is expected,
not a bug. The skip rule guards against edge chips that are half-ocean/half-nodata.

Usage (run in project root with venv active):
    python scripts/preprocess_s1_ots.py
...
# ── Paths (hardcoded to this specific product — no glob, no ._* risk) ───────
...
AOIS = {
    ...
    "ots_aoi_b": (53.50, 54.05, 5.00, 5.60),  # eastern edge tightened: was 5.80
}

CHIP_SIZE    = 512
STRIDE       = 256
BLACK_THRESH = 0.80   # skip chip if fraction of zero pixels exceeds this
PREVIEW_PX   = 1400   # target longest side for preview image
...
cal_pix_arr = np.vstack(cal_pix_rows)
...
with rasterio.open(TIFF_PATH) as ds:
    ...
    for aoi_name, (row_off, col_off, row_end, col_end) in aoi_windows.items():
        ...
        # Step D: Read DN → calibrate → dB → clip → uint8
        ...
        skip_counts[aoi_name] = n_skipped
        print(f"   Chips saved:   {n_saved}")
        print(f"   Chips skipped: {n_skipped}  (>{BLACK_THRESH:.0%} black — "
              f"{'expected for interior ocean' if n_skipped == 0 else 'edge chips'})")


# Step G: inventory
print("\n── Chip inventory ─────────────────────────────────────────────────────")
all_scenes = {
    "maasvlakte":  CHIPS_DIR / "maasvlakte",
    "ots_aoi_a":   CHIPS_DIR / "ots_aoi_a",
    "ots_aoi_b":   CHIPS_DIR / "ots_aoi_b",
}
for name, d in all_scenes.items():
    n = sum(1 for f in d.glob("*.png")
            if not f.name.startswith("._")) if d.exists() else 0
    print(f"  {name:15s}: {n:4d} chips  →  {d}")

print(f"\nOutput root: {CHIPS_DIR}")
print("\nNext step: chip pre-screening + labeling workflow")
print("  python scripts/screen_chips.py ots_aoi_a")
print("  python scripts/screen_chips.py ots_aoi_b")
