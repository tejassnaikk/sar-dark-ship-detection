...
# ── Target product (locked for Week 2) ────────────────────────────────────
PRODUCT_ID   = "4774839e-104f-4a44-8c8f-b9b14847ceb7"
PRODUCT_NAME = "S1A_IW_GRDH_1SDV_20250430T055840_20250430T055905_058982_07507C_E57F.SAFE"
...
with session.get(download_url, stream=True, timeout=60) as resp:
    ...
    with open(zip_path, "wb") as f:
        for chunk in resp.iter_content(chunk_size=1 << 20):  # 1 MB chunks
            ...
            if downloaded - last_print > 100 << 20 or pct >= 100:  # every 100 MB
...
final_size = sum(f.stat().st_size for f in output_path.rglob("*") if f.is_file()) / 1e9
print(f"\nDone. {output_path}")
print(f"  Size on disk: {final_size:.2f} GB")
print(f"\nKey files:")
for p in sorted(output_path.rglob("*.tiff"))[:4]:
